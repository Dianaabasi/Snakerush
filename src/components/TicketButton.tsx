...
export default function TicketButton({ fid, livesToMint, ethPrice, onSuccess }: TicketButtonProps) {
  ...
  // Attempt Auto-Connect for Farcaster Users if not connected
  useEffect(() => {
  ...
  const handleConnect = () => {
    ...
    // LOGIC FIX: Check environment based on FID presence
    if (fid && injected) {
      // If we have an FID, we are likely in Farcaster -> Use Injected (Warpcast Wallet)
      connect({ connector: injected });
    } else if (coinbase) {
      // Otherwise (Base App / Web) -> Use Coinbase SDK
