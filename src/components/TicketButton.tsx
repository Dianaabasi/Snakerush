...
export default function TicketButton({ fid, livesToMint, ethPrice, onSuccess }: TicketButtonProps) {
  ...
  const handleConnect = () => {
    const farcaster = connectors.find(c => c.id === 'farcasterWallet'); // 🟣 NEW
    const coinbase = connectors.find(c => c.id === 'coinbaseWalletSDK');
    const injected = connectors.find(c => c.id === 'injected');

    if (farcaster) {
      console.log("Connecting with Farcaster Wallet...");
      return connect({ connector: farcaster });
    }
    ...
    if (connectors.length > 0) {
      console.log("Fallback: connecting with first available connector...");
